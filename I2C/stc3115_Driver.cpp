...
RAMData_TypeDef STC3115::RAMData; // now it is private internal static of the class
...
*******************************************************************************/
int STC3115::Write(int NumberOfBytes, uint8_t RegAddress , uint8_t *TxBuffer)
{
    // Result
    int res;
    unsigned int res_;
    ...
    // Device address as vector
    std::vector<uint8_t> addr;
    addr.push_back(STC3115_SLAVE_ADDRESS_SHIFTED);

    // Construct transmit message
    std::vector<uint8_t> tx_msg(TxBuffer, TxBuffer + NumberOfBytes); // need to check if it copies the data!
...
*******************************************************************************/
int STC3115::Read(int NumberOfBytes, uint8_t RegAddress , uint8_t *RxBuffer)
{
    // Result
    int res;
    unsigned int res_;
    ...
    std::vector<unsigned char> rx_msg = i2c.recData();
    std::copy(rx_msg.begin(), rx_msg.end(), RxBuffer);
...
*******************************************************************************/
int STC3115::ReadByte(uint8_t RegAddress)
{
    int value;
    uint8_t data[2];
    int res;

    res = Read(1, RegAddress , data);
...
*******************************************************************************/
int STC3115::WriteByte(uint8_t RegAddress, uint8_t Value)
{
    int res;
    uint8_t data[2];

    data[0]= Value;
    res = Write(1, RegAddress, data);
...
*******************************************************************************/
int STC3115::ReadWord(uint8_t RegAddress)
{
    int value;
    unsigned char data[2];
    int res;

    res = Read(2, RegAddress , data);
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::WriteWord(uint8_t RegAddress, int Value)
{
    int res;
    unsigned char data[2];
...
* Return         : 16-bit value, or 0 if error
*******************************************************************************/
int STC3115::ReadBytes(unsigned char *data,uint8_t RegAddress,int nbr)
{
    int res;

    res = Read(nbr, RegAddress , data);
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::WriteBytes(uint8_t* data,uint8_t RegAddress,int nbr)
{
    int res;

    res = Write(nbr, RegAddress , data);
...
* Return         : 0
*******************************************************************************/
void STC3115::SetParam(STC3115_ConfigData_TypeDef *ConfigData)
{
    int value;

    /* set GG_RUN=0 before changing algo parameters */
    WriteByte(STC3115_REG_MODE,STC3115_VMODE);

    /* init OCV curve */
    WriteBytes(static_cast<uint8_t*>(ConfigData->OCVOffset), STC3115_REG_OCVTAB, OCVTAB_SIZE);

    /* set alm level if different from default */
    if (ConfigData->Alm_SOC !=0 )
        WriteByte(STC3115_REG_ALARM_SOC,static_cast<uint8_t>(ConfigData->Alm_SOC*2));
    if (ConfigData->Alm_Vbat !=0 )
    {
        value = static_cast<int>(static_cast<long>(ConfigData->Alm_Vbat << 9) / VoltageFactor); /* LSB=8*2.2mV */
        WriteByte(STC3115_REG_ALARM_VOLTAGE, static_cast<uint8_t>(value));
    }
    
    /* relaxation timer */
    if (ConfigData->Rsense !=0 )
    {
        value= static_cast<int>(static_cast<long>(ConfigData->RelaxCurrent << 9) / (CurrentFactor / ConfigData->Rsense));   /* LSB=8*5.88uV */
        WriteByte(STC3115_REG_CURRENT_THRES,static_cast<uint8_t>(value));
    }

    /* set parameters if different from default, only if a restart is done (battery change) */
    if (RAMData.reg.CC_cnf !=0 ) WriteWord(STC3115_REG_CC_CNF,RAMData.reg.CC_cnf);
    if (RAMData.reg.VM_cnf !=0 ) WriteWord(STC3115_REG_VM_CNF,RAMData.reg.VM_cnf);

    WriteByte(STC3115_REG_CTRL,0x03);  /*   clear PORDET, BATFAIL, free ALM pin, reset conv counter */
    WriteByte(STC3115_REG_MODE, STC3115_GG_RUN | (STC3115_VMODE * static_cast<uint8_t>(ConfigData->Vmode)) | (STC3115_ALM_ENA * ALM_EN));  /*   set GG_RUN=1, set mode, set alm enable */
...
*******************************************************************************/
int STC3115::Startup(STC3115_ConfigData_TypeDef *ConfigData)
{
    int res;
    int ocv;
    ...
    SetParam(ConfigData);  /* set STC3115 parameters  */

    /* rewrite ocv to start SOC with updated OCV curve */
    WriteWord(STC3115_REG_OCV,ocv);
...
* Return         : 
*******************************************************************************/
int STC3115::Restore(STC3115_ConfigData_TypeDef *ConfigData)
{
    int res;
    ...
    WriteWord(STC3115_REG_SOC,RAMData.reg.HRSOC);
...
*******************************************************************************/
int STC3115::conv(short value, unsigned short factor)
{
    int v;

    v = static_cast<int>((static_cast<long>(value) * factor) >> 11);
    v = (v+1)/2;
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::ReadBatteryData(STC3115_BatteryData_TypeDef *BatteryData)
{
    ...
    BatteryData->Current = conv(static_cast<int16_t>(value), CurrentFactor/RSENSE);  /* result in mA */

    /* voltage */
    value=data[9]; value = (value<<8) + data[8];
    value &= 0x0fff; /* mask unused bits */
    if (value>=0x0800) value -= 0x1000;  /* convert to signed value */
    value = conv(static_cast<int16_t>(value),VoltageFactor);  /* result in mV */
    ...
    value = conv(static_cast<int16_t>(value),VoltageFactor);
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::ReadRamData(unsigned char *RamData)
{
    return ReadBytes(RamData, STC3115_REG_RAM, RAM_SIZE);
}
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::WriteRamData(uint8_t *RamData)
{
    return WriteBytes(RamData, STC3115_REG_RAM, RAM_SIZE);
}
...
* Return         : CRC calue
*******************************************************************************/
uint8_t STC3115::CalcRamCRC8(uint8_t *data, int n)
{
...
* Return         : none
*******************************************************************************/
void STC3115::InitRamData(STC3115_ConfigData_TypeDef *ConfigData)
{
    int index;
    ...
    RAMData.reg.CC_cnf = static_cast<int16_t>(ConfigData->CC_cnf);
    RAMData.reg.VM_cnf = static_cast<int16_t>(ConfigData->VM_cnf);
    /* update the crc */
    UpdateRamCRC();
}
...
*******************************************************************************/
int STC3115::Init(STC3115_ConfigData_TypeDef *ConfigData,STC3115_BatteryData_TypeDef *BatteryData)
{
    int res,loop;
    ...
    for(loop=0;loop<16;loop++)
    {
        ...
        if(OCVOffset[loop] < -127) OCVOffset[loop] = -127;
        ConfigData->OCVOffset[loop] = static_cast<uint8_t>(OCVOffset[loop]);
    }
    ...
    ReadRamData(RAMData.db);

    if ( (RAMData.reg.TstWord != RAM_TSTWORD) || (CalcRamCRC8(RAMData.db,RAM_SIZE)!=0) )
    {
        /* RAM invalid */
        InitRamData(ConfigData);
        res=Startup(ConfigData);  /* return -1 if I2C error or STC3115 not present */
    }
    else
    {
        ...
        {
            res=Restore(ConfigData); /* recover from last SOC */
        }
    }
...
int STC3115::Reset()
{
    ...
    res = WriteRamData(RAMData.db);
    if(res != OK_STC) return res;
    /* reset STC3115*/
    res = WriteByte(STC3115_REG_CTRL, STC3115_PORDET);  /*   set soft POR */
...
*******************************************************************************/
int STC3115::Task(STC3115_ConfigData_TypeDef *ConfigData,STC3115_BatteryData_TypeDef *BatteryData)
{
    int res;
    ...
    if ((BatteryData->status & STC3115_GG_RUN) == 0)
    {
        ...
        if(RAMData.reg.STC3115_Status == (STC3115_RUNNING | STC3115_POWERDN))
            Restore(ConfigData);  /* if RUNNING state, restore STC3115*/
        else
        {
            Startup(ConfigData);  /* if INIT state, initialize STC3115*/
        }
    ...
    res = ReadBatteryData(BatteryData);
    ...
    else /*STC3115 running */
    {
        ...
        if ((BatteryData->status & STC3115_VMODE) == 0) /* mixed mode only*/
        {
            ...
            if ((BatteryData->status & STC3115_VMODE) == 0) /*running in mixed mode*/
            {
                ...
                {
                    BatteryData->SOC = 990;
                    WriteWord(STC3115_REG_SOC,50688);   /* 99% */
                }
            }
    ...
    /* save SOC */
    RAMData.reg.HRSOC = static_cast<int16_t>(BatteryData->HRSOC);
    RAMData.reg.SOC = static_cast<int8_t>((BatteryData->SOC+5)/10);
    UpdateRamCRC();
    WriteRamData(RAMData.db);
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::SetPowerSavingMode()
{
    int res;
    ...
    res = WriteByte(STC3115_REG_MODE, static_cast<uint8_t>(res | STC3115_VMODE));
    if (res!= OK_STC) return (res);
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::StopPowerSavingMode()
{
    int res;
    ...
    if (VMODE != MIXED_MODE) return (!OK_STC);
...
int STC3115::AlarmSet()
{
    ...
    res = WriteByte(STC3115_REG_MODE, static_cast<uint8_t>(res | STC3115_ALM_ENA));
    if (res!= OK_STC) return (res);
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::AlarmSetVoltageThreshold(STC3115_ConfigData_TypeDef *ConfigData, int VoltThresh)
{
    int res;
    int value;
    ...
    value= static_cast<int>(static_cast<long>(ConfigData->Alm_Vbat << 9) / VoltageFactor); /* LSB=8*2.2mV */
    res = WriteByte(STC3115_REG_ALARM_VOLTAGE, static_cast<uint8_t>(value));
    if (res!= OK_STC) return (res);
...
* Return         : error status (OK, !OK)
*******************************************************************************/
int STC3115::AlarmSetSOCThreshold(STC3115_ConfigData_TypeDef *ConfigData, int SOCThresh)
{
    int res;

    ConfigData->Alm_SOC = SOCThresh;
    res = WriteByte(STC3115_REG_ALARM_SOC, static_cast<uint8_t>(ConfigData->Alm_SOC*2));
    if (res!= OK_STC) return (res);
